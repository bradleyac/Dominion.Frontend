...
export const Resources = () => {
  ...
  return (
    <div className={styles.resources}>
      ...
      <KingdomButton />
      <ResourceIcon type="Actions" lit={phase === "Action"} title="End Action Phase" disabled={!isCurrentPlayer || phase !== "Action"} counts={[actions, villagers]} onClick={() => signalrConnector?.endActionPhase(gameId)} />
      <ResourceIcon type="Coins" lit={phase === "BuyOrPlay"} title="Play All Treasures" disabled={!isCurrentPlayer || !["Action", "BuyOrPlay"].includes(phase)} counts={[coins, coffers]} onClick={() => signalrConnector?.playAllTreasures(gameId)} />
      <ResourceIcon type="Buys" lit={phase === "Buy"} disabled={true} counts={[buys]} />
      <ResourceIcon type="Cleanup" lit={phase === "Cleanup"} title="End Turn" disabled={!isCurrentPlayer} counts={[]} onClick={() => signalrConnector?.endTurn(gameId)} />
    </div>
  )
};

...
export function getCardClickAction
  ...
  if (choice && choice.$type === "select") {
    ...
    if (filter?.from === zone && (selectedCards.includes(card.id) || (cardInstanceId && selectedCards.includes(cardInstanceId)))) {
      // This card is currently selected, so allow deselection
      return "deselect";
    }

    // If the supply pile is empty, don't allow selection.
    if (filter.from === "Supply" && !state.gameState.kingdomState.supply.find(cps => cps.cardId === card.id && cps.count > 0)) {
      return "none";
    }
  ...
  switch (turnState.phase) {
    case "Action": {
      switch (zone) {
        case "Hand": return (resources.actions > 0 && card.types.includes("Action")) || card.types.includes("Treasure") ? "play" : "none";
        default: return "none";
      }
    }
    case "BuyOrPlay": {
      switch (zone) {
        ...
        case "Supply": return resources.buys > 0 && card.cost <= resources.coins && state.gameState.kingdomState.supply.find(cps => cps.cardId === card.id && cps.count > 0) ? "buy" : "none";
        default: return "none";
      }
    }
    case "Buy": {

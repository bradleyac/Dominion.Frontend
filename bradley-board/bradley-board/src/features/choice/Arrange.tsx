...
export const Arrange = (): JSX.Element => {
  ...
  return (<>
    {choice && choice.$type === "arrange" && <ArrangeImpl choice={choice as PlayerArrangeChoice} cards={cards} />}
  </>);
}
...
export const IndexedZone = ({ index, arrangeCallback, children }: PropsWithChildren<{ index: number, arrangeCallback: (cardInstanceId: string, newIndex: number) => void }>): JSX.Element => {
  return <div className={styles.zone} onDragOver={e => e.preventDefault()} onDrop={e => arrangeCallback(e.dataTransfer.getData("text/plain"), index)}>
    {children}
  </div>
}